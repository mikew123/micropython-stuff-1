...
class Robo24Window:
    ...
    def build_root_window(self):
        ...
        button.register_click_handler(self.open_robo24_window)

    def open_robo24_window(self):
        Robo24ChooseWindow(self.window_manager, self.display)


class Robo24ChooseWindow:
    def __init__(self, window_manager, display):
        self.window_manager = window_manager
        self.display = display
        self.build_robo24_choose_window()
        self.window_manager.push_window(self.choose_window)

    def build_robo24_choose_window(self):
        self.choose_window = Window(self.display, 'ROBO24 Choose')
        ...
        label = VisualLabel(Point(0, 20), 'ROBO24', font_32, True, Color.TITLE)
    ...
    def clicked_mission(self):
        print('Robo24 Missions')
        Robo24MissionWindow(self.window_manager, self.display)

    def clicked_sensors(self):
        print('Robo24 Sensors')
        Robo24SensorWindow(self.window_manager, self.display)

    def clicked_motors(self):
        print('Robo24 Motors')
        Robo24MotorWindow(self.window_manager, self.display)


class Robo24MissionWindow:
    def __init__(self, window_manager, display):
        self.window_manager = window_manager
        self.display = display
    ...
    def build_robo24_mission_run_window(self, mission_name):
...
class Robo24EditMissionWindow:
    ...
    def build_robo24_choose_window(self):
        self.choose_window = Window(self.display, 'Robo24 Edit Mission')
    ...
    def clicked_edit_cone_sets(self):
        Robo24TestCanvasWindow(self.window_manager, self.display)


class Robo24TestCanvasWindow:
    def __init__(self, window_manager, display):
        self.window_manager = window_manager
        self.display = display
        self.build_main_window()
...
class Robo24EditConeSetWindow:
    ...
    def build_main_window(self):
        cone_sets = [
            Robo24ConeSet('Start', 'BC#1'),
            Robo24ConeSet('Start', 'BC#2'),
            Robo24ConeSet('Start', 'Finish'),
            Robo24ConeSet('BC#1', 'BC#2'),
            Robo24ConeSet('BC#2', 'BC#1'),
            Robo24ConeSet('BC#1', 'Finish'),
            Robo24ConeSet('BC#2', 'Finish')
        ]
        windows = []
...
class Robo24MotorWindow:
    ...
    def build_window_for_motor(self, motor_name):
        ...
        if (motor_name!='Claw') :
            ...
            button_y = 60
            button = VisualButton(Rectangle(Point(0, button_y), Point(160, 35)), 'OPEN', font_25, Color.BUTTON)
            middle_view.add_component(button)
            button.register_click_handler(self.clicked_claw_motor, "OPEN")
            
            button_y += 40
            button = VisualButton(Rectangle(Point(0, button_y), Point(160, 35)), 'CLOSE', font_25, Color.BUTTON)
            middle_view.add_component(button)
            button.register_click_handler(self.clicked_claw_motor, "CLOSE")

        return window

    def clicked_claw_motor(self, open_close:str) ->None:
        self.telemetry = Telemetry(ROBO24_MAC_ADDRESS, WIFI_CHANNEL_NUM)
